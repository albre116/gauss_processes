mlegp_modified <- function (X, Z, constantMean = 1, nugget = NULL, nugget.known = 0, 
          ...
          BFGS.h = 1e-10, seed = 0, verbose = 1, parallel = FALSE) 
{
  ...
  if (!parallel) {
    l = lapply(Zlist, mlegp2_modified, XX = X, orig.XX = orig.X, nugget = nugget, 
  ...
  if (parallel) {
    ...
    l = sfLapply(Zlist, mlegp2_modified, XX = X, orig.XX = orig.X, 
